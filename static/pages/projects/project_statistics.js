...
function GetScanName(){
     $.ajax({
         type: "GET",
         url: "/projects/api/getprojectname",
         data: {id: id},
         dataType: "json",
         success: function (data) {
             if (data.status == 0) {
                 $('#brProject').text(data.object)
             }
         },
     })
}
...
function ajaxRequest(params) {
    $.ajax({
        type: "GET",
        url: "/vuln/api/getcurrentprojectvuln",
        data: params.data,
        dataType: "json",
        success: function(data) {
            if(data.status == 0){
...
function DrawChartScanResult(){
    function GetData() {
        return $.ajax({
            url:'/scans/api/getscansvulns?projectID='+id,
            success: function (data) {
                if(data.status ==0)
                    rawData = data.rows;
            }
...
function DrawChartOSStat(){
    function GetData() {
        return $.ajax({
            url:'/hosts/api/gethostsos?projectID='+id,
            success: function (data) {
                if(data.status ==0)
                    rawData = data.rows;
            }
...
function DrawChartServiceStat(){
    function GetData() {
        return $.ajax({
            url:'/services/api/getservicesvuln?sortName=total&projectID='+id,
            method: "GET",
            success: function (data) {
                if(data.status ==0)
                    rawData = data.object;
...
function DrawChartHostVuln(){
    function GetData() {
        return $.ajax({
            url:'/hosts/api/gethostscurrentvuln?projectID='+id,
            success: function (data) {
                if(data.status ==0)
                    rawData = data.rows;
            }
    ...
    $.when(GetData()).done(function (results){
        ...
        var ctx = document.getElementById("hostVulnChart").getContext("2d");
        new Chart(ctx, {
            type: 'bar',
            data: {
                labels: labels,
            ...
            options: {
                ...
                title: {
                    display: true,
                    text: 'Statistics of Vulnerabilities By Host'
                },
                scales: {
                    xAxes: [{ stacked: true }],
                    yAxes: [{ stacked: true }]
